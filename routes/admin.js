...
// Admin Detailed Analysis Dashboard
router.get("/analysis", async (req, res) => {
  try {
    // Get all employees and departments
    const departments = await Department.find({ isActive: true }).populate("hods", "name")
    const employees = await User.find({
      role: { $in: ["employee", "hod"] },
      isActive: true,
    }).populate("department", "name")
    ...
    for (const dept of departments) {
      ...
      const deptReviews = allReviews.filter((r) => r.department._id.toString() === dept._id.toString())

      if (deptReviews.length > 0) {
        const scores = deptReviews.map((r) => r.effectiveScore)
        ...
        departmentAnalytics.push({
          ...
          maxScore,
          minScore,
          hodNames: dept.hods.map((h) => h.name).join(", "),
        })
      } else {
        departmentAnalytics.push({
          name: dept.name,
    ...
    departmentAnalytics.sort((a, b) => Number.parseFloat(b.avgScore) - Number.parseFloat(a.avgScore))
    ...
    if (allReviews.length > 0) {
      const scores = allReviews.map((r) => r.effectiveScore)
      overallAvgScore = (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(2)
    }
    ...
    const topPerformers = employees
      .map((emp) => {
        const empReviews = allReviews.filter((r) => r.employee._id.toString() === emp._id.toString())
        if (empReviews.length > 0) {
          const avgScore = (
            empReviews.map((r) => r.effectiveScore).reduce((a, b) => a + b, 0) / empReviews.length
          ).toFixed(2)
          return { employee: emp, avgScore, reviewCount: empReviews.length }
        }
        return null
      })
      .filter((x) => x !== null)
    ...
    res.render("admin/detailed-analysis", {
      departmentAnalytics,
      overallAvgScore,
      totalReviews,
      totalEmployees,
...
router.get("/questions/:id/edit", async (req, res) => {
  try {
    const question = await Question.findById(req.params.id).populate("department", "name")
    if (!question) {
      return res.status(404).render("error", { message: "Question not found" })
    }

    const departments = await Department.find({ isActive: true })
    res.render("admin/edit-question", { question, departments })
  } catch (error) {
    console.error("Get question error:", error)
    res.status(500).render("error", { message: "Error loading question" })
  }
})
...
router.post("/questions/:id/edit", async (req, res) => {
  try {
    ...
    // Update input type and options for self-assessment questions
    if (questionType === "self-assessment") {
    ...
    await question.save()
    console.log("Question updated:", question.text)

    res.redirect("/admin/questions")
  } catch (error) {
    console.error("Edit question error:", error)
    res.status(500).render("error", { message: "Error updating question" })
  }
})
...
router.post("/questions/:id/delete", async (req, res) => {
  try {
    ...
    await question.save()
    console.log("Question deleted:", question.text)
