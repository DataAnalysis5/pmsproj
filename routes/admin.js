...
router.get("/dashboard", async (req, res) => {
  try {
    ...
    const currentQuarterReviews = await Review.countDocuments({ quarter: currentQuarter })

    // Average rating for current quarter (calculated from aggregated reviews) - Updated for 6-point scale
    let avgRating = 0
    try {
...
// Add Question - Updated categories
router.post(
  "/questions",
  [
  ...
  async (req, res) => {
    try {
      ...
      const { text, category, department } = req.body
      ...
      console.log("Question created:", newQuestion.text, "for department:", newQuestion.department || "Global")
...
// Helper function to get quarterly trends
  ...
  try {
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      try {
        ...
        const reviewCount = await Review.countDocuments({ quarter: quarterStr })
