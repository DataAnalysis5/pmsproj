...
router.get("/reviews", async (req, res) => {
  try {
    ...
    // Self-assessments are always submitted for the current month by employees,
    // but they should be visible to HOD if questions are set to "All Months" (month=null)
    const selfAssessments = await SelfAssessment.find({
      employee: { $in: allEmployeesToReviewIds },
      month: getCurrentMonth(), // Self-assessments are submitted for current month
    }).populate("answers.question", "text category inputType month")

    // Create map and filter based on question month settings
    selfAssessments.forEach((assessment) => {
      // Check if ALL questions in this assessment are either:
      // 1. Set to "All Months" (month: null), OR
      // 2. Set to the selected month
      const isApplicable = assessment.answers.some((answer) => {
        const questionMonth = answer.question.month
        return questionMonth === null || questionMonth === selectedMonth
      })

      // Only add to map if assessment is applicable to selected month
      if (isApplicable) {
