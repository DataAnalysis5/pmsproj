...
router.get("/reviews", async (req, res) => {
  try {
    ...
    const allReviewsByReviewer = await Review.find({ reviewer: req.session.user._id })
      .populate("employee", "name employeeId role")
      .populate("answers.question", "text category")
      .sort({ reviewDate: -1 })

    // Create a map of employee ID to their most recent review by this reviewer
    const previousReviewsMap = {}
    allReviewsByReviewer.forEach((review) => {
    ...
    res.render("hod/reviews", {
      ...
      currentMonth,
      department: req.userDepartment,
      previousReviewsMap, // Pass the map of previous reviews
      selfAssessmentsMap: JSON.stringify(selfAssessmentsMap),
    })
  } catch (error) {
    console.error("Reviews page error:", error)
    res.status(500).render("error", {
      message: "Error loading reviews page: " + error.message,
      user: req.session.user || null,
    })
  }
})
